...
/// McQuay A/C model numbers
enum mcquay_ac_remote_model_t {
  G4AAP = 1,  // 
};
...
#endif
#if SEND_MCQUAY
  void sendMcQuay(const uint64_t data, const uint16_t nbits = kMcQuayBits,
                const uint16_t repeat = kMcQuayDefaultRepeat);
  void sendMcQuay(const uint8_t data[], const uint16_t nbytes = kMcQuayStateLength,
                const uint16_t repeat = kMcQuayDefaultRepeat);
#endif
#if SEND_GOODWEATHER
