...
rfir::ttl::IRSend::IRSend(uint16_t IRsendPin, bool inverted, bool use_modulation)
...
void rfir::ttl::IRSend::begin() {
    pinMode(pin, OUTPUT);
    ledOff(); 
}

/// Turn off the IR LED.
void rfir::ttl::IRSend::ledOff() {
    digitalWrite(pin, outputOff);
}

void rfir::ttl::IRSend::ledOn() {
    digitalWrite(pin, outputOn);
}


uint32_t rfir::ttl::IRSend::calcUSecPeriod(uint32_t hz, bool use_offset) {
    if (hz == 0) hz = 1;  
...
void rfir::ttl::IRSend::enableIROut(uint32_t freq, uint8_t duty) {
    if (modulation) {
...
void rfir::ttl::IRsend::_delayMicroseconds(uint32_t usec) {
...
uint16_t rfir::ttl::IRSend::mark(uint16_t usec) {
...
void rfir::ttl::IRSend::space(uint32_t time) {
    ledOff();
    if (time == 0) return;
...
int8_t rfir::ttl::IRSend::calibrate(uint16_t hz) {
    if (hz < 1000)  
        hz *= 1000;
    periodOffset = 0;  
    enableIROut(hz);
...
void rfir::ttl::IRSend::sendRaw(const uint16_t buf[], const uint16_t len,
                     const uint16_t hz) {
